...
@NamedQueries({
    ...
    @NamedQuery(name = "ViewLectureNotTaken.findByIdProgramCourse", query = "SELECT v FROM ViewLectureNotTaken v WHERE v.idProgram = :program AND v.idCourse = :course AND v.semester = :sem AND  MOD( v.semester, 2 )<> 0"),
    ...
    @NamedQuery(name = "ViewLectureNotTaken.findByIdFaculty", query = "SELECT v FROM ViewLectureNotTaken v WHERE v.idFaculty = :idFaculty AND MOD( v.semester, 2 )<> 0 ORDER BY v.calDate, v.division"),
